    ...
    public static MinecraftModInformation fromFile(Path path) {
        ...
        if (modInfoEntry != null) {
                ...
                if (element.isJsonArray()) {
                    ...
                    for (JsonElement jObject : jArray) {
                        ...
                        if (desiredFields.stream().allMatch(info::has)) {
                            modInformation = new MinecraftModInformation(
            ...
            if (modInformation == null) {
                Logger.debug(
                    String.format(
                        "%s - Could not find the desired fields in the mod information file: %s",
            ...
            Logger.debug(
                String.format(
                    "%s - Could not find a mod information file that matches: %s",
                    path.getFileName().toString(),
