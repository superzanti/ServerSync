	...
	public void run() {
		...
		try {
			...
			for(String singleFile : fileTree){
				...
				SyncClient.updateScreenWorking((int)(5+(currentPercent/percentScale)),"Checking server's " + singleFile);
				...
				if(f.exists() && !f.isDirectory()){
					...
					if(!Md5.md5String(f).equals(serverChecksum)){
						if (ServerSyncRegistry.IGNORE_LIST.contains(singleFile.replace('\\',  '/'))){
							...
							SyncClient.updateScreenWorking((int)(5+(currentPercent/percentScale)),"Updating " + singleFile);
		    ...
		    for (String singleFile : allList){
		    	currentPercent++;
				SyncClient.updateScreenWorking((int)(5+(currentPercent/percentScale)),"Checking client's " + singleFile);
				...
				if (ServerSyncRegistry.IGNORE_LIST.contains(singleFile.replace('\\',  '/'))){
					...
					if(doesExist.equalsIgnoreCase("false")){
						...
						SyncClient.updateScreenWorking((int)(5+(currentPercent/percentScale)),"Deleting client's " + singleFile);
